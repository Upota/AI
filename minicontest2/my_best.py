...
class ExpectiMaxAgent(CaptureAgent):
  """
  alpha-beta 가지치기를 적용한 모델로 부터 Expecti-Max agent를 구성하였다.
  feature를 얻는 것과 가중치는 alpha-beta 가지치기를 적용한 모델과 동일하다.
  """
  ...
  def value(self, gameState, orderIdx, current_depth):                  # expecti-Max 를 위한 세 함수 들이다.
  ...
  """
  def max_layer(self, gameState, orderIdx, current_depth):     # max를 골라내는 레이어이다.
  ...
  def exp_layer(self, gameState, orderIdx, current_depth):          # expectation을 계산하는 레이어다.
    agent = self.order[orderIdx]
    LegalMoves = gameState.getLegalActions(agent)
    LegalMoves.remove('Stop') 
    p = 1/(len(LegalMoves))                                   # 확률은 uniform distribution을 따른다.
    exp_val = 0
    exp_act = []
...
class OffensiveReflexAgent(ExpectiMaxAgent):
  """
  이 부분은 앞서 기술한 alpha-beta 가지치기 모델과 동일하다.
  따라서 동일한 주석들이 존재한다.

  기존 reflex에서는 getfeature함수 내부에 전달받은 액션을 이용해 예상되는 다음 게임 상태로부터 정보를 얻어 가치를 계산하는데,
  이 모델에서는 전달받는 gameState가 이미 미래 상황에 대한 정보를 가지고 있기에 gameState를 활용하여 가치를 계산하는 점에서 차이점이 존재한다.
  """
  def evaluate(self, gameState, action):
    """
 ...
 # 상대방 진영의 유령과의 거리를 계산하였다.
    ...
    if len(ghosts) > 0:
      ...
      if closestGhost.scaredTimer == 0:
        if mindist == 1:                           # 유령이 너무 가까우면 도망친다.
          features['ghostISclose'] = 9
          onReturn = 1
        elif mindist < 5:                          # 유령이 가까이 있음을 경고
          features['ghostISclose'] = 5
      else:
    ...
    if prev != None:
      ...
      if prevState.numCarrying > 0 and numReturned - prevState.numReturned == prevState.numCarrying:
        self.bouns += 1
        features['bonus'] = self.bouns        # 위의 코드와 마찬가지로 dot의 반환에 성공하였을 때, 행동양식이 다시 공격모드로 바뀌므로 그에 대한 부가 가치를 부여한다.
    ...
    if len(foodList) > 0:                                          # 자신의 위치에서 모든 음식들의 거리 정보와 음식의 가까운 유령과의 거리 정보를 복합하여 나아가야 할 길을 알려준다.
      ...
      if len(ghosts) > 0:
        ...
        weightedDist = np.array(distToDot) - 0.5 * np.array(distDotnGhost)
      else:
  ...
  def getWeights(self, gameState, action, onReturn):
    ...
    """
    if onReturn:
      return {'Score': 10, 'onAttack': -10, 'eatCapsule': 15, 'numCarrying': 10, 'numReturned': 10, 'ghostDistance': 1, 'ghostISclose': -3, 'dist': -2, 'eatGhost': 10, 'bonus': 80}

    return {'Score': 10, 'reverse': -15, 'onAttack': 10, 'eatCapsule': 15, 'numCarrying': 10, 'numReturned': 10, 'ghostISclose': -2.5, 'eatGhost': 10, 'distanceToFood': -2, 'bonus': 80}
